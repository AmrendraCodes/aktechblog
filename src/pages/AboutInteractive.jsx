import { Link } from 'react-router-dom';
import "./AboutInteractive.css";

const About = () => {
    const skills = [
        { name: 'React', level: 90 },
        { name: 'TypeScript', level: 85 },
        { name: 'Node.js', level: 80 },
        { name: 'CSS/Tailwind', level: 88 },
        { name: 'MongoDB', level: 75 },
        { name: 'Git', level: 85 },
    ];

    const experiences = [
        {
            id: 1,
            title: 'Senior Developer',
            company: 'Tech Company',
            period: '2022 - Present',
            description: 'Leading frontend development and mentoring junior developers.'
        },
        {
            id: 2,
            title: 'Full Stack Developer',
            company: 'Startup Inc',
            period: '2020 - 2022',
            description: 'Built scalable web applications using React and Node.js.'
        },
        {
            id: 3,
            title: 'Junior Developer',
            company: 'Digital Agency',
            period: '2018 - 2020',
            description: 'Developed responsive websites and learned modern web technologies.'
        },
    ];

    return (
        <div className="about-page">
            {/* Breadcrumb */}
            <div className="about-breadcrumb-section">
                <div className="about-breadcrumb-container">
                    <nav className="about-breadcrumb" aria-label="Breadcrumb">
                        <ol className="breadcrumb-list">
                            <li className="breadcrumb-item">
                                <Link
                                    to="/"
                                    className="breadcrumb-link"
                                >
                                    <svg className="icon-sm mr-2" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-7z"></path>
                                    </svg>
                                    Home
                                </Link>
                            </li>
                            <li className="breadcrumb-separator">
                                <svg className="icon-md text-muted" fill="currentColor" viewBox="0 0 20 20">
                                    <path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd"></path>
                                </svg>
                                <span className="breadcrumb-current">About</span>
                            </li>
                        </ol>
                    </nav>
                </div>
            </div>

            <div className="about-content-wrapper">

                {/* Hero Section */}
                <div className="about-hero">
                    <div className="about-avatar-wrapper">
                        <div className="about-avatar">
                            JD
                        </div>
                    </div>
                    <h1 className="about-hero-title">
                        I'm Amrendra kumar
                    </h1>
                    <p className="about-hero-desc">
                        I'm a passionate full-stack developer with expertise in building modern,
                        scalable web applications. I love turning ideas into reality through clean code.
                    </p>
                </div>

                {/* About Main Content */}
                <div className="about-grid">

                    {/* Left Column - About Text */}
                    <div className="about-card about-story">
                        <h2 className="about-section-title">
                            My Story
                        </h2>
                        <div className="about-text-content">
                            <p>
                                Hello! I'm a software developer with a passion for creating beautiful,
                                functional, and user-centered digital experiences. With over 5 years of
                                experience in the industry, I've worked on a diverse range of projects.
                            </p>
                            <p>
                                My journey into web development started when I built my first website
                                at age 16. Since then, I've been constantly learning and evolving with
                                the ever-changing technology landscape.
                            </p>
                            <p>
                                When I'm not coding, you can find me exploring new technologies,
                                contributing to open-source projects, or sharing my knowledge through
                                blog posts and mentoring.
                            </p>
                        </div>

                        {/* Contact Info */}
                        <div className="about-contact-info">
                            <h3 className="about-subsec-title">Get in Touch</h3>
                            <div className="about-contact-links">
                                <a href="mailto:your@email.com" className="contact-link-item">
                                    <svg className="icon-sm mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                                    </svg>
                                    your@email.com
                                </a>
                                <a href="https://github.com/yourusername" className="contact-link-item">
                                    <svg className="icon-sm mr-3" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                                    </svg>
                                    GitHub
                                </a>
                                <a href="https://linkedin.com/in/yourusername" className="contact-link-item">
                                    <svg className="icon-sm mr-3" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
                                    </svg>
                                    LinkedIn
                                </a>
                            </div>
                        </div>
                    </div>

                    {/* Right Column - Skills */}
                    <div className="about-right-col">
                        <div className="about-card about-skills">
                            <h2 className="about-section-title">
                                Skills & Expertise
                            </h2>
                            <div className="skills-list">
                                {skills.map((skill) => (
                                    <div key={skill.name} className="skill-item">
                                        <div className="skill-info">
                                            <span className="skill-name">{skill.name}</span>
                                            <span className="skill-level">{skill.level}%</span>
                                        </div>
                                        <div className="skill-bar-bg">
                                            <div
                                                className="skill-bar-fill"
                                                style={{ width: `${skill.level}%` }}
                                            ></div>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        </div>

                        {/* Fun Facts */}
                        <div className="about-card about-facts">
                            <h3 className="about-subsec-title">Fun Facts</h3>
                            <ul className="facts-list">
                                <li className="fact-item">
                                    <span className="fact-icon">☕</span>
                                    <span>Coffee enthusiast - 4 cups a day minimum!</span>
                                </li>
                                <li className="fact-item">
                                    <span className="fact-icon">🎮</span>
                                    <span>Love gaming in my free time</span>
                                </li>
                                <li className="fact-item">
                                    <span className="fact-icon">📚</span>
                                    <span>Always learning new technologies</span>
                                </li>
                                <li className="fact-item">
                                    <span className="fact-icon">🌍</span>
                                    <span>Dream to work remotely while traveling</span>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>

                {/* Experience Timeline */}
                <div className="about-card about-experience">
                    <h2 className="experience-title">
                        Professional Experience
                    </h2>
                    <div className="timeline">
                        {experiences.map((exp) => (
                            <div key={exp.id} className="timeline-item">
                                <div className="timeline-dot"></div>

                                <div className="timeline-content">
                                    <div className="timeline-header">
                                        <h3 className="timeline-role">{exp.title}</h3>
                                        <span className="timeline-period">{exp.period}</span>
                                    </div>
                                    <p className="timeline-company">{exp.company}</p>
                                    <p className="timeline-desc">{exp.description}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                </div>

                {/* CTA Section */}
                <div className="about-cta-section">
                    <div className="cta-card">
                        <h2 className="cta-title">
                            Let's Work Together!
                        </h2>
                        <p className="cta-text">
                            I'm always interested in hearing about new projects and opportunities.
                        </p>
                        <div className="cta-buttons">
                            <Link to="/contact" className="cta-btn primary-cta">
                                Contact Me
                            </Link>
                            <Link to="/projects" className="cta-btn secondary-cta">
                                View Projects
                            </Link>
                        </div>
                    </div>
                </div>

            </div>
        </div>
    )
}

export default About;
